    ...
    <b-row>
      <b-col sm=12>
        <b-table
          ...
          responsive="sm" hover
          :items="smartNodesToTableItems()"
          :fields="tableFields"
          head-variant="light"
        >
          <!-- <div slot="label" slot-scope="item">
            <span class="avatar-status" v-bind:class="{ 'bg-success': item.status == 'Enabled',  'bg-warning': item.status !== 'Enabled'}"> {{ item.label }}</span>
          </div> -->
          </b-table>
      </b-col>
    </b-row>
    <b-button to="/smart-nodes/new">New SmartNode</b-button>
  </div>
</template>
...
export default {
  ...
  methods: {
    smartNodesToTableItems () {
      return this.smartNodes.map(node => {
        return {
          label: node.label,
          address: node.address,
          status: node.status,
          sent: node.content.sent,
          received: node.content.received,
          balance: node.content.balance,
          _cellVariants: { status: node.status === 'Enabled' ? 'success' : 'error' }
        }
      })
    },
    getSmartNodes () {
      api.service('smart-nodes').find()
        .then(results => {
          console.log(results.data)
          this.smartNodes = results.data
        })
